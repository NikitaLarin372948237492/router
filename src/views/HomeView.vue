    ...
    <div class="home">
      ...
      <div class="grid-container">
        ...
        <div class="grid-item" v-for="(item, index) in clothes.woman" v-bind:key="index">
          ...
          <p>{{ item.name }}</p>
          <p>{{ item.price }} $</p>
          <button class="add-to-card" @click="funcs.addCartItem(index, 'woman')">Buy</button>
        </div>
        <div class="grid-item" v-for="(item, index) in clothes.kids" v-bind:key="index">
          <img class="image" :src=item.src />
          <p>{{ item.name }}</p>
          <p>{{ item.price }} $</p>
          <button class="add-to-card" @click="funcs.addCartItem(index, 'kids')">Buy</button>
        </div>
      </div>
      <div class="footer">
        <div class="wrapf">shop 2021</div>
      </div>
    </div>
  </body>
</template>
